...
class CollectiveStage:
    ...
    request in the stage before passing the completed values and continuation to the metric.
    Branches that need to inspect one another's completed values belong in the same stage and share
    its continuation; independent computations use separate stages.
...
class TensorMetric(ABC):
    ...
    def resume(self, values: Sequence[MetricTensor], continuation: object) -> MetricStep:
        ...
        Args:
            values: Collective results in request order, with each targeted axis marked as
                replicated.
            continuation: Opaque state supplied by the completed stage.
...
class LogicalReductionMetric(TensorMetric):
    ...
    def resume(self, values: Sequence[MetricTensor], continuation: object) -> MetricStep:
        ...
        Args:
            values: Completed branch reductions in request order.
    ...
    def complete(
        ...
        while steps:
            ...
            for stage in stages:
                ...
                next_steps.append(
                    metric.resume(tuple(completed_values[offset:end]), stage.continuation)
                )
                offset = end
            steps = next_steps
